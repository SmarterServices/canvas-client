...
class CanvasRequester {
  ...
  _refreshAccessToken() {
    ...
    return this
      ...
      .then(response => {
        ...
        if (typeof this.updateAccessToken === 'function') {
          return this
  ...
  _parsePagination(header) {
    ...
    pageMap.perPage = /per_page=([0-9]+)/.exec(currentPage)[1];
    // get total page count from https://...?page=10&...
//    pageMap.totalPages = /(?!_)page=([0-9]+)/.exec(lastPage)[1];
    // JasonF: Last page is not always present per the docs: https://canvas.instructure.com/doc/api/file.pagination.html
    if(lastPage){
      pageMap.totalPages = /(?!_)page=([0-9]+)/.exec(lastPage)[1];
    }else{
      pageMap.totalPages = 10;
    }
